...
:START
if not exist "%phpDir%/%phpExe%" goto NOPHP
echo Killing all other running PHP processes
taskkill /F /IM "%phpExe%"
...
for /l %%a in (%basePort%,1,%maxPort%) do echo Starting listener on port %%a && start /b "" "%phpDir%/%phpExe%" -b %listenHost%:%%a
echo %totalProcesses% total PHP processes started
echo PHP FastCGI pool ready

:WATCHDOG
for /f "tokens=1,*" %%a in ('tasklist ^| find /I /C "%phpExe%"') do set running=%%a
...
:NOPHP
echo %phpExe% not found in %phpDir%
pause
exit